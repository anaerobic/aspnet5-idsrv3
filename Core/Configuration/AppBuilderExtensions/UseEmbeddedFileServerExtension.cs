...
namespace Owin
{
    internal static class UseEmbeddedFileServerExtension
    {
        public static IAppBuilder UseEmbeddedFileServer(this IAppBuilder app)
        {
            ...
            {
                throw new Exception("location-less assembly compilation not supported :(");
            }

            app.UseFileServer(new FileServerOptions
            {
                RequestPath = new PathString("/assets"),
                FileSystem = new EmbeddedResourceFileSystem(assembly, AssetManager.HttpAssetsNamespace)
            });
